...
/***************************************************
  ...
  The algorithm is comparing two different indexes in
  the array and looks out which one is higher/lower.

  diff		=	the number between the two comparing 
				indexes
  shrink 	=  	the value in which the diff is divided 
				in each iteration
  swapped	=	if there has been an index swapped

***************************************************/


static void comb_sort (int * arr, int len)
{
	float shrink = 1.3f;
	int swap = 0;
	int i, j;
	...
	while((diff > 1) || (swapped))
	 {
        if (diff > 1)
		{
            diff = (int)((float)diff / shrink);
        }

        swapped = 0;

		for (i = 0, j = diff; j < len; i++, j++)
		{
            if (arr[i] > arr[j]) 
			{
                swap = arr[i];
